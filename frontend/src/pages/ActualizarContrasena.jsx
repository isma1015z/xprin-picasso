...
export function ActualizarContrasena() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      if (!session) {
        setError('⚠️ ACCESO DENEGADO: No se ha detectado una sesión de recuperación válida. Utiliza el enlace enviado a tu correo o solicita uno nuevo.');
  ...
  const handleSubmit = async (e) => {
    ...
    if (passwordStrength.score < 50) { setError('La contraseña es demasiado débil. Debe tener al menos 8 caracteres e incluir números.'); return; }
      ...
      <div className={`flex min-h-screen bg-brand-white transition-opacity duration-700 ${isPrinting ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-20 xl:px-32 bg-brand-white/90 backdrop-blur-xl shadow-[-20px_0_50px_rgba(0,0,0,0.1)] z-10 relative border-l border-brand-white/20">
          <div className="mx-auto w-full max-w-sm lg:max-w-md animate-fade-in-up">
            ...
            {error && (
              <div className="mb-6 p-4 bg-brand-red/10 border-l-4 border-brand-red text-brand-red text-xs font-bold rounded-sm animate-fade-in-up leading-relaxed">{error}</div>
            )}

            {!isAuthorized && !checkingSession ? (
              <div className="mt-8">
                <button onClick={() => { setIsPrinting(true); setTimeout(() => navigate('/recuperar'), 1500); }}
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="group">
                  ...
                  <div className="relative">
                    <input id="newPassword" type="password" required placeholder="••••••••"
                      value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
                      className="block w-full bg-brand-white border-brand-gray/30 rounded-sm py-4 px-4 text-brand-dark shadow-sm ring-1 ring-inset ring-brand-gray/20 placeholder:text-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-red/50 focus:bg-white transition-all outline-none text-sm sm:leading-6" />
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <div className="h-1 w-full bg-brand-gray/10 rounded-full overflow-hidden">
                      <div className={`h-full transition-all duration-700 ease-out ${passwordStrength.color}`} style={{ width: `${passwordStrength.score}%` }}></div>
                    </div>
                  </div>
                </div>
