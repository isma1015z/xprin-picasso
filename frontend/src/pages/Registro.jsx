...
export function Registro() {
  ...
  useEffect(() => {
    ...
    if (currentMediaIndex === 0) {
      ...
      if (video1Ref.current) { video1Ref.current.currentTime = 0; video1Ref.current.play().catch(() => {}); }
    } else if (currentMediaIndex === 2) {
      if (video2Ref.current) { video2Ref.current.currentTime = 0; video2Ref.current.play().catch(() => {}); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        email, password,
        options: {
          data: { full_name: fullName, company, data_validation_hash: dataValidationHash },
      ...
      <div className={`flex min-h-screen bg-brand-white transition-opacity duration-700 ${isPrinting ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-20 xl:px-32 bg-brand-white/90 backdrop-blur-xl shadow-[-20px_0_50px_rgba(0,0,0,0.1)] z-10 relative border-l border-brand-white/20 overflow-y-auto">
          <div className="mx-auto w-full max-w-sm lg:max-w-md animate-fade-in-up py-8">
            <Link to="/" onClick={(e) => { e.preventDefault(); setIsPrinting(true); setTimeout(() => navigate('/'), 1500); }}
            ...
            {error && (
              <div className="mb-4 p-3 bg-brand-red/10 border-l-4 border-brand-red text-brand-red text-xs font-bold animate-fade-in-up">{error}</div>
            )}
              ...
              <div className="space-y-8 animate-fade-in-up">
                <div className="bg-brand-gray/10 p-8 rounded-sm border border-brand-gray/20 text-center">
                  ...
                  <p className="text-sm text-brand-carbon/70 font-medium leading-relaxed">
                    Para activar tu acceso al motor <strong>PICASSO</strong>, haz clic en el enlace enviado a <strong>{email}</strong>.
                  </p>
                </div>
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <input id="fullName" type="text" required placeholder="Juan Pérez" value={fullName} onChange={(e) => setFullName(e.target.value)}
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <input id="email" type="email" required placeholder="juan.perez@empresa.com" value={email} onChange={(e) => setEmail(e.target.value)}
                      className="block w-full bg-brand-white border-brand-gray/30 rounded-sm py-3.5 px-4 text-brand-dark shadow-sm ring-1 ring-inset ring-brand-gray/20 placeholder:text-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-red/50 focus:bg-white transition-all outline-none sm:text-sm sm:leading-6" />
                    <div className="absolute bottom-0 left-0 h-0.5 bg-brand-red w-0 group-focus-within:w-full transition-all duration-500"></div>
                  </div>
                </div>
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <input id="company" type="text" required placeholder="XPRIN Solutions" value={company} onChange={(e) => setCompany(e.target.value)}
                      className="block w-full bg-brand-white border-brand-gray/30 rounded-sm py-3.5 px-4 text-brand-dark shadow-sm ring-1 ring-inset ring-brand-gray/20 placeholder:text-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-red/50 focus:bg-white transition-all outline-none sm:text-sm sm:leading-6" />
                    <div className="absolute bottom-0 left-0 h-0.5 bg-brand-red w-0 group-focus-within:w-full transition-all duration-500"></div>
                  </div>
                </div>
                ...
                {/* Dirección */}
                <div className="group">
                  <label htmlFor="address" className="block text-xs font-bold text-brand-carbon/60 uppercase tracking-widest mb-2 transition-colors group-focus-within:text-brand-red">Dirección de la empresa</label>
                  <div className="relative">
                    <input id="address" type="text" required placeholder="Calle de la Innovación, N° 45" value={address} onChange={(e) => setAddress(e.target.value)}
                      className="block w-full bg-brand-white border-brand-gray/30 rounded-sm py-3.5 px-4 text-brand-dark shadow-sm ring-1 ring-inset ring-brand-gray/20 placeholder:text-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-red/50 focus:bg-white transition-all outline-none sm:text-sm sm:leading-6" />
                    <div className="absolute bottom-0 left-0 h-0.5 bg-brand-red w-0 group-focus-within:w-full transition-all duration-500"></div>
                  </div>
                </div>
                ...
                {/* Teléfono */}
                <div className="group">
                  <label htmlFor="phone" className="block text-xs font-bold text-brand-carbon/60 uppercase tracking-widest mb-2 transition-colors group-focus-within:text-brand-red">Teléfono de contacto</label>
                  <div className="relative">
                    <input id="phone" type="tel" required placeholder="+34 600 123 456" value={phone} onChange={(e) => setPhone(e.target.value)}
                      className="block w-full bg-brand-white border-brand-gray/30 rounded-sm py-3.5 px-4 text-brand-dark shadow-sm ring-1 ring-inset ring-brand-gray/20 placeholder:text-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-red/50 focus:bg-white transition-all outline-none sm:text-sm sm:leading-6" />
                    <div className="absolute bottom-0 left-0 h-0.5 bg-brand-red w-0 group-focus-within:w-full transition-all duration-500"></div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 py-2">
                  <input id="terms" type="checkbox" required checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)}
                    className="h-4 w-4 rounded border-brand-gray/50 text-brand-red focus:ring-brand-red cursor-pointer accent-brand-red" />
                  <label htmlFor="terms" className="text-[10px] font-bold text-brand-carbon/60 uppercase tracking-tighter cursor-pointer select-none">
                ...
                <div className="mt-8 pt-6 border-t border-brand-gray/20">
                  <div className="bg-brand-white/50 backdrop-blur-sm rounded-xl p-6 border border-brand-gray/10 hover:border-brand-red/20 transition-all group/card shadow-sm">
                    ...
                    <button type="submit" disabled={loading}
                      ...
                      <span className="relative z-10 tracking-widest">{loading ? 'SOLICITANDO ACCESO...' : 'SOLICITAR ACCESO'}</span>
            ...
            <div className="mt-10 text-center text-sm text-brand-carbon opacity-80">
              ¿Ya eres miembro?{' '}
              <Link to="/login" onClick={handleLoginClick} className="font-semibold leading-6 text-brand-cyan hover:opacity-80 transition-colors">
                Inicia sesión aquí
              </Link>
            </div>
          </div>
