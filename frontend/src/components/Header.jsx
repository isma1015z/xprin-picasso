...
export function Header({ theme, toggleTheme, isMobile = false, mobileMenuOpen = false, toggleMobileMenu = () => { } }) {
  ...
  async function handleExport({ embedImagen = true, preview = false } = {}) {
    setExportMenu(false)
    if (spotCount === 0) {
      setError('Asigna al menos un spot a una capa antes de exportar.')
      return
    }
    setExportandoPDF(true)
    setError(null)
    try {
      const { lastFile } = useStore.getState()
      ...
      if (lastFile) fd.append('imagen', lastFile)
        ...
        {imagenUrl && (
          <>
            ...
            <div className="relative inline-block w-[210px] max-md:hidden">
              ...
              />
              <img src={theme === 'dark' ? lapizBlanco : lapiz} alt="Editar" className="w-5 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none transition-all duration-200" />
            </div>
          </>
        )}
      </div>
          ...
          <div className="relative max-md:hidden">
            ...
            >
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-accent text-white hover:bg-accent-hover transition-colors cursor-pointer"
              ...
              <button
                ...
                className="flex items-center justify-center px-2 py-2 bg-accent hover:bg-accent-hover text-white border-l border-white/20 transition-colors cursor-pointer"
            ...
            {exportMenu && (
              <div className="absolute top-full right-0 mt-1 z-50 w-56 bg-surface rounded-lg shadow-xl border border-border-strong overflow-hidden" onMouseLeave={() => setExportMenu(false)}>
        ...
        <div className={`absolute top-full left-0 right-0 z-40 bg-surface border-b border-border-strong shadow-lg px-3 py-2
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => fileInputRef.current?.click()} disabled={cargando}
              className="h-10 w-full flex items-center justify-center gap-1.5 px-3 rounded-md text-xs font-medium border border-border-light bg-surface-elevated text-secondary hover:bg-surface-hover hover:text-primary hover:border-border-strong transition-all duration-200 disabled:opacity-50 cursor-pointer">
              {cargando ? <svg className="w-3.5 h-3.5 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg> : <Upload size={14} />}
            ...
            {imagenUrl && (
              <button onClick={() => handleExport({ embedImagen: true })}
                className={`h-10 w-full flex items-center justify-center gap-1.5 px-3 rounded-md text-xs font-medium transition-colors cursor-pointer bg-accent text-white hover:bg-accent-hover
                  ${spotCount === 0 ? 'opacity-40 pointer-events-none' : ''}
                  ${exportandoPDF ? 'opacity-70 pointer-events-none' : ''}`}>
          ...
          {imagenUrl && (
            <div className="mt-2 relative">
              <button onClick={() => setExportMenu((v) => !v)}
                className="h-9 w-full flex items-center justify-center gap-1.5 rounded-md text-[11px] font-medium border border-border-light bg-surface-elevated text-secondary hover:bg-surface-hover hover:text-primary transition-colors cursor-pointer">
                Opciones de exportación
                <ChevronDown size={12} className={`transition-transform ${exportMenu ? 'rotate-180' : ''}`} />
              </button>
              {exportMenu && (
                <div className="absolute top-full left-0 mt-1 z-50 w-full bg-surface rounded-lg shadow-xl border border-border-strong overflow-hidden" onMouseLeave={() => setExportMenu(false)}>
                  <button onClick={() => handleExport({ embedImagen: true })} className="w-full text-left px-4 py-2.5 text-sm text-primary hover:bg-surface-hover transition-colors cursor-pointer">
                    <div className="font-medium">Con imagen</div>
                    <div className="text-xs text-muted">PDF completo para revisión</div>
                  </button>
          ...
          {errorMsg && <div className="text-[11px] text-accent truncate mt-1" title={errorMsg}>{errorMsg}</div>}
        </div>
      )}
    </header>
  )
