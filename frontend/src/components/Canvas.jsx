...
export function Canvas() {
  ...
  const usedTexturas = [...new Set(capas.filter(c => c.spot === 'texture' && c.texturaId).map(c => c.texturaId))]
      ...
      >
        <div className="flex flex-col items-center justify-center">
          <button
            ...
            disabled={cargando}
            className="flex flex-col items-center justify-center w-[400px] h-[300px] border-2 border-dashed border-border-strong rounded-xl bg-surface-elevated/50 hover:bg-surface transition-colors duration-200 select-none cursor-pointer disabled:opacity-50"
          >
            {cargando ? (
            ...
            >
              <defs>
                {usedTexturas.map(texId => {
                  ...
                  if (!tex || !tex.thumb) return null
                  return (
                    <pattern key={tex.id} id={`tex-${tex.id}`} patternUnits="userSpaceOnUse" width="400" height="400">
                      <image href={tex.thumb} width="400" height="400" preserveAspectRatio="xMidYMid slice" opacity="0.75" />
                    </pattern>
                  )
                })}
              </defs>
